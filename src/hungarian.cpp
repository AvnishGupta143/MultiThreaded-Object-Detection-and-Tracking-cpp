...
void HungarianAlgorithm::step2b(int *assignment, double *distMatrix, bool *starMatrix, bool *newStarMatrix, bool *primeMatrix, bool *coveredColumns, bool *coveredRows, int nOfRows, int nOfColumns, int minDim)
{
	...
	if ((nOfCoveredColumns == minDim) || count >100)
	{
		/* algorithm finished */
...
{   count++;
	bool zerosFound;
	int row=0, col=0, starCol=0;
	zerosFound = true;
	while (zerosFound)
	{
		zerosFound = false;
		for (col = 0; col<nOfColumns; col++)
			if (!coveredColumns[col] || count >100)
				for (row = 0; row<nOfRows; row++)
					if ((!coveredRows[row]) && (fabs(distMatrix[row + nOfRows*col]) < DBL_EPSILON) || count>100)
					{
						/* prime zero */
						...
						if (starCol == nOfColumns || count > 100) /* no starred zero found */
						{
							/* move to step 4 */
